...
public class GrpcIoServerImpl extends GrpcServerImpl implements GrpcIoServer {
  ...
  // Method descriptors registered via callHandler(MethodDescriptor, Handler). Guarded by the
  // instance lock, which is the same monitor super.callHandler(ServiceMethod, Handler) uses,
  // so the routing map and this list stay in sync under concurrent (un)registration.
  private final List<MethodDescriptor<?, ?>> callHandlerMethods = new ArrayList<>();
  ...
  public List<Service> services() {
    ...
    List<MethodDescriptor<?, ?>> snapshot;
    synchronized (this) {
      if (callHandlerMethods.isEmpty()) {
        return services;
      }
      snapshot = new ArrayList<>(callHandlerMethods);
    }
    ...
    for (MethodDescriptor<?, ?> methodDesc : snapshot) {
