...
You can compress request messages by setting a compression algorithm when creating the client, or per message.
...
Decompression is achieved by client if it has access to proper decompressors and if the proper decompression algorithm is
registered in `GrpcClientCompressionOptions`.
