...
[WARNING]
====
The plugin generates the transcoding definitions even if the definitions are not defined and by default schema for
the transcoding for each method which does not have a definition is of http method `POST` and the path is in the format
