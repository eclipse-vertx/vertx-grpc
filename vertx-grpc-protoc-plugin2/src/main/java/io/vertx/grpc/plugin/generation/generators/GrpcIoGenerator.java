...
public class GrpcIoGenerator extends AbstractCodeGenerator<GenerationContext> {
  ...
  public GenerationResult generate(GenerationContext context) {
    ...
    for (ServiceDescriptor service : context.getServices()) {
      try {
        ...
        String fileName = context.getOptions().getServicePrefix() + service.getName() + "GrpcIo.java";
        ...
        return GenerationResult.failure(List.of(
          new GenerationError("Failed to generate gRPC IO for enableService: " + service.getName(), e, generationType)
        ));
      }
    }
