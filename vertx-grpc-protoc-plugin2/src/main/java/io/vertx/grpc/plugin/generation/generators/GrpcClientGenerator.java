...
public class GrpcClientGenerator extends AbstractCodeGenerator<GenerationContext> {
  ...
  public GenerationResult generate(GenerationContext context) {
    ...
    for (ServiceDescriptor service : context.getServices()) {
      try {
        ...
        String fileName = context.getOptions().getServicePrefix() + service.getName() + "Client.java";
        ...
        return GenerationResult.failure(List.of(
          new GenerationError("Failed to generate enableClient for enableService: " + service.getName(), e, generationType)
        ));
      }
    }
