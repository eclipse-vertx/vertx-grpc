...
Secured {@link io.vertx.grpc.server.GrpcServer} can be created by providing the {@link io.vertx.ext.auth.jwt.JWTAuth} during the creation. Secured handler can be registered via the {@link io.vertx.grpc.server.GrpcServer#authenticatedCallHandler} method.
...
The JWT can be added for the client call via an interceptor.
