  ...
  private class Adapter implements Handler<Message<Object>> {
    ...
    public void handle(Message<Object> message) {
      ...
      if (serviceMethod.type().streaming()) {
        dispatchStreaming(message, serviceMethod);
      } else {
        dispatchUnary(message, serviceMethod);
      }
    }

    private <Req, Resp> void dispatchUnary(Message<Object> message, ServiceMethod<Req, Resp> serviceMethod) {
    ...
    private <Req, Resp> void dispatchStreaming(Message<Object> message, ServiceMethod<Req, Resp> serviceMethod) {
      ContextInternal context = (ContextInternal) vertx.getOrCreateContext();
      WireFormat wireFormat = WireFormat.valueOf(message.headers().get(EventBusHeaders.WIRE_FORMAT));
      ...
      stream.listenHandler(v -> {
        ...
        // the opening request body is the first request message
        request.handleMessage(GrpcMessage.message("identity", wireFormat, payload));
      });
    }
  }
}
